...
echo Publishing Uninstall.exe ...
dotnet publish Uninstall\PTor.Uninstall.csproj -c Release -r win-x64 --self-contained true -p:PublishSingleFile=true -p:EnableCompressionInSingleFile=true -p:Version=%NEWVER% -p:AssemblyVersion=%NEWVER%.0 -p:FileVersion=%NEWVER%.0 -p:InformationalVersion=%NEWVER% -o "Release\v%NEWVER%"
if errorlevel 1 (
