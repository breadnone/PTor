...
//   3b.clears the Windows logon startup entries (HKCU/HKLM Run value "PTor"
//      plus Startup-folder shortcuts) so nothing relaunches after uninstall,
...
bool restoreOnly = cli.Any(a => a.Equals("--restore-only", StringComparison.OrdinalIgnoreCase));
    ...
    || a.Equals("/?", StringComparison.OrdinalIgnoreCase)))
{
    ...
    Console.WriteLine("registry keys, logon startup entries, own WinDivert service, %AppData%\\PTor, %ProgramData%\\PTor).");
    Console.WriteLine("Usage: Uninstall [--dry-run] [--yes|-y] [--no-reboot] [--log <file>] [--restore-only]");
    ...
    Console.WriteLine("  --restore-only  headless network-only restore for the boot safety net:");
    Console.WriteLine("                  restores proxy/env/DNS if stale, then exits (no prompts,");
    Console.WriteLine("                  no reboot, nothing deleted). Exits quietly while PTor runs.");
    return 0;
}
string? logPath = null;
...
// --- 3b. logon startup entries: Run values + Startup-folder shortcuts ---
// PTor registers HKCU\...\Run "PTor" when "Start on startup" is on. HKLM is
// never written by the app, but an old/manual install may have put it there,
// so both go (compare-before-delete is unnecessary: the value NAME is ours by
// construction — no other program registers itself as "PTor").
// The boot safety net ("PTorNetRestore" task + one-time RunOnce entry)
// goes too — the box is clean by definition once this finishes, so the net
// must not linger.
DeleteRunValue(Registry.CurrentUser, "HKCU\\...\\Run \"PTor\"");
DeleteRunValue(Registry.LocalMachine, "HKLM\\...\\Run \"PTor\"");
try { BootRestore.Remove(); Say("Boot safety net (logon restore task/entry): removed."); } catch { }
DeleteStartupShortcut(Environment.SpecialFolder.Startup, "%AppData% Startup\\PTor.lnk");
DeleteStartupShortcut(Environment.SpecialFolder.CommonStartup, "%ProgramData% Startup\\PTor.lnk");
...
// --- 4b. HTTPS-inspection leftovers: our CA trust (every reachable store)
// + leaf key + Firefox auto-config ---
// %AppData%\PTor deletion below removes the CA files, but trust lives in
// the cert stores and the leaf key in CNG — all must go explicitly. This
// runs elevated, so it covers the current user AND LocalMachine AND every
// local user's Firefox auto-config (matched by our exact CA subject, so a
// foreign CA is never touched).
if (dryRun)
{
    Say("MITM CA: would remove PTor's CA from user + machine Trusted Root, delete its leaf key, and remove its Firefox auto-config for all users.");
}
else
{
    try { Say("MITM CA: " + MitmCaManager.RemoveTrustForAllUsers()); }
    catch (Exception ex) { Say("MITM CA: cleanup failed (" + ex.Message + ")."); }
}
...
// Headless network-only restore for the boot safety net (logon task / Run
// entry): restores proxy/env/DNS when PTor died uncleanly, then exits with
// no prompts, no reboot, nothing deleted. Exits quietly while PTor runs
// (the live owner manages its own state — never fight it). Self-cleaning:
// the task/entry is removed once no MANAGED state remains, so the net
// exists only while it is needed. Always returns 0 (a logon task must
// never "fail" loudly). Never throws.
int RestoreOnly()
{
    try
    {
        try
        {
            if (Process.GetProcessesByName("PTor").Length > 0)
            {
                Say("PTor is running — live owner keeps its network state (nothing to do).");
                FlushLog();
                return 0;
            }
        }
        ...
        if (StillStale())
        {
            if (dryRun) { Say("Stale PTor network state found (would restore; --dry-run changes nothing)."); FlushLog(); return 0; }
            Say(Do("Proxy", () => new SystemProxyManager().Disable()));
            Say(Do("Env  ", () => new UserEnvManager().Disable()));
            Say(Do("DNS  ", () => new SystemDnsManager().Disable()));
            FlushDns();
            ...
            if (StillManaged())
            {
                // Partial (typically unelevated DNS): keep the net so the
                // next logon retries. Elevated repair (PTor as admin,
                // rescue-internet.bat, or full uninstall) clears the rest.
                Say("Partial restore — managed state remains (system DNS needs admin). Safety net kept for the next logon.");
                FlushLog();
                return 0;
            }
            Say("Stale PTor network state restored.");
            try
            {
                if (new SystemDnsManager().CountStrandedLoopback() > 0)
                    Say("Note: some interfaces still point at loopback DNS with no PTor backup — left alone (could be your own resolver); rescue-internet.bat fixes those with explicit consent.");
            }
            catch { }
        }
        else
        {
            Say("No stale PTor network state — nothing to do.");
        }
        if (!dryRun)
        {
            try { BootRestore.Remove(); } catch { }
            Say("Boot safety net removed (state clean).");
        }
        else
        {
            Say("Boot safety net left alone (--dry-run changes nothing).");
        }
    }
    catch (Exception ex)
    {
        try { Say("Restore-only issue: " + ex.Message); } catch { }
    }
    try { FlushLog(); } catch { }
    return 0;
}

// Managed markers (actionable + PTor-attributable). Stranded loopback with
// no marker is deliberately NOT included: it is indistinguishable from the
// user's own local resolver and only the explicit-consent rescue script
// touches it.
bool StillManaged()
{
    try { if (new SystemProxyManager().WasLeftManaged()) return true; } catch { }
    try { if (new UserEnvManager().GetSnapshot().Managed) return true; } catch { }
    try { if (new SystemDnsManager().WasLeftManaged()) return true; } catch { }
    return false;
}

bool StillStale()
{
    try { if (StillManaged()) return true; } catch { }
    try { if (new SystemDnsManager().CountStrandedLoopback() > 0) return true; } catch { }
    return false;
}
...
void DeleteRunValue(RegistryKey hive, string display)
{
    const string run = @"Software\Microsoft\Windows\CurrentVersion\Run";
    try
    {
        using var k = hive.OpenSubKey(run, writable: true);
        if (k == null) { Say($"{display}: Run key unreadable — nothing to delete."); return; }
        var cur = k.GetValue("PTor", null) as string;
        if (string.IsNullOrWhiteSpace(cur)) { Say($"{display}: absent — nothing to delete."); return; }
        if (dryRun) { Say($"{display}: would delete (points at: {cur})."); return; }
        k.DeleteValue("PTor", throwOnMissingValue: false);
        Say($"{display}: deleted (was: {cur}).");
    }
    catch (Exception ex) { Say($"{display}: could not delete ({ex.Message})."); }
}

void DeleteStartupShortcut(Environment.SpecialFolder folder, string display)
{
    try
    {
        string dir;
        try { dir = Environment.GetFolderPath(folder); }
        catch (Exception ex) { Say($"{display}: folder unreadable ({ex.Message})."); return; }
        if (string.IsNullOrWhiteSpace(dir)) { Say($"{display}: folder unknown — nothing to delete."); return; }
        // Only our own shortcut name — never wildcard the Startup folder.
        var lnk = Path.Combine(dir, "PTor.lnk");
        if (!File.Exists(lnk)) { Say($"{display}: absent — nothing to delete."); return; }
...
{
    // Same normalization as EnforcementCheckpoint (single ground truth
    // there): NT prefixes (\\?\, \??\) + \SystemRoot\ + env vars + full path.
