...
namespace PTor
{
    ...
    public class TorProcessManager : IDisposable
    {
        ...
        // True when the token is a relay fingerprint ($FP or bare 40-hex,
        // with optional =name/~name suffix) rather than a nickname.
        // Fingerprints are stable identifiers; nicknames can be taken over
        // by another operator or resolve ambiguously, so a nickname pin can
        // silently start using a different relay (an IP hop with no warning
        // from tor). Never throws.
        public static bool IsFingerprintToken(string? token)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(token)) return false;
        ...
        // Why a pin may STILL change exit IP, in plain words. Tor's pin
        // options only constrain WHICH relays may be used — they do not
        // freeze Tor's circuit lifecycle: circuits still die naturally
        // (idle timeout, exit closes, network flap) and Tor builds fresh
        // ones. The exit IP only stays put when a single exit relay is
        // pinned: with no exit pinned Tor picks a fresh exit per new
        // circuit (normal hopping), and with several pinned Tor spreads
        // circuits across all of them (hopping inside the set). Pure,
        // never throws; empty list = pin holds one IP (barring relay death,
        // which the auto-recovery path reports explicitly).
        public static List<string> PinStabilityWarnings(
            IEnumerable<string>? entry, IEnumerable<string>? middle, IEnumerable<string>? exit)
        {
            var warnings = new List<string>();
            try
            {
                var e = SanitizePinnedNodes(entry);
                var m = SanitizePinnedNodes(middle);
                var x = SanitizePinnedNodes(exit);
                if (x.Count == 0)
                    warnings.Add("no EXIT relay pinned — Tor picks a fresh exit whenever a circuit is rebuilt, " +
                        "so the exit IP WILL keep hopping. Pin exactly one exit fingerprint to hold one IP " +
                        "(entry/middle pins alone never stop exit hops).");
                else if (x.Count > 1)
                    warnings.Add(x.Count + " exits pinned — Tor spreads circuits across all of them, so the IP hops " +
                        "WITHIN your set. Pin exactly ONE exit fingerprint for a fixed IP (more exits = redundancy " +
                        "at the cost of hops).");
                if (e.Count > 1)
                    warnings.Add(e.Count + " entries pinned — Tor may use any of them (entry churn; use one for maximum stability).");
                if (m.Count > 1)
                    warnings.Add(m.Count + " middles pinned — Tor spreads circuits across all of them (middle churn; one is enough).");
                var nicks = 0;
                try
                {
                    foreach (var t in e.Concat(m).Concat(x))
                        if (!IsFingerprintToken(t)) nicks++;
                }
                catch { }
                if (nicks > 0)
                    warnings.Add(nicks + " nickname(s) instead of $FINGERPRINT — nicknames can be taken over or resolve " +
                        "ambiguously and silently move your pin to a different relay. Prefer fingerprints " +
                        "(Config → 'Use current circuit' fills them in).");
            }
            catch { }
            return warnings;
        }
            ...
            IEnumerable<string>? pinnedExitNodes = null)
        {
            ...
            // An entry-only pin under bridges constrains NOTHING (the entry
            // line is dropped below and no StrictNodes is emitted): it must
            // not override the circuit lifetime either — otherwise an
            // ineffective pin silently forces a ~32-year dirtiness on a run
            // it doesn't constrain. Single rule shared with the block below
            // and with the engine's BootstrapPinActive.
            var pinConstrains = pinActive && (!bridging || (pinMiddle.Count + pinExit.Count) > 0);
            var effectiveDirtiness = pinConstrains ? PinnedCircuitDirtinessSec : Math.Max(60, dirtinessSec);
            var torrc = $@"
...
";
            ...
            if (pinConstrains)
            {
                ...
                torrc += "StrictNodes 1\n";
                // Tor's default (EnforceDistinctSubnets 1) refuses a
                // hand-picked entry/middle/exit triple that shares a /16
                // or family — the pinned circuit then never builds and
                // the run flaps between dead circuits (which reads as
                // "the pin keeps hopping"). The user explicitly chose
                // these relays, so the subnet rule is lifted on pinned
                // runs only. Unpinned runs keep Tor's default.
                torrc += "EnforceDistinctSubnets 0\n";
            }
            else
            {
                // Ineffective pin (entry-only under bridges): the entry
                // line stays dropped AND the saved geo still applies —
                // an unconstrained run must not lose the user's region.
                if (pinActive && pinEntry.Count > 0 && bridging)
                    torrc += "# EntryNodes <dropped for this run: the bridge is the entry — tor refuses UseBridges+EntryNodes>\n";
