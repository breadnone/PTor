...
namespace PTor
{
    ...
    public static class AppTrafficMonitor
    {
        ...
        public record RemoteEndpoint(string Ip, int Port, string State, bool Loopback);

        // Proof surface for the secret diagnostics panel: every TCP row
        // (established + SYN-sent) owned by one pid, with remote endpoints.
        // Read-only table snapshot; never throws (empty list on any failure).
        public static List<RemoteEndpoint> SnapshotRemotes(int pid)
        {
            var out_ = new List<RemoteEndpoint>();
            try
            {
                ReadTcpRemotes(out_, AF_INET, pid);
                ReadTcpRemotes(out_, AF_INET6, pid);
            }
            catch { }
            return out_;
        }

        static string TcpStateName(uint state) => state switch
        {
            2 => "LISTEN",
            3 => "SYN-SENT",
            4 => "SYN-RCVD",
        ...
        static void ReadTcpRemotes(List<RemoteEndpoint> out_, int family, int pid)
        {
            uint Query(IntPtr p, ref int l) =>
                GetExtendedTcpTable(p, ref l, false, family, TCP_TABLE_CLASS.TCP_TABLE_OWNER_PID_ALL, 0);
            var buf = AllocTable(Query);
            ...
            try
            {
                ...
                if (family == AF_INET)
                {
                    ...
                    for (uint i = 0; i < count; i++)
                    {
                        ...
                        var loop = IsV4Loopback(r.remoteAddr);
                        out_.Add(new RemoteEndpoint(ip, PortFromDword(r.remotePort), TcpStateName(r.state), loop));
                    }
                }
                else
                {
                    ...
                    for (uint i = 0; i < count; i++)
                    {
                        ...
                        var ip = new IPAddress(r.remoteAddr).ToString();
                        out_.Add(new RemoteEndpoint(ip, PortFromDword(r.remotePort), TcpStateName(r.state), IsV6Loopback(r.remoteAddr)));
                    }
                }
            }
            finally { Marshal.FreeHGlobal(buf); }
        ...
        public static int? GetTcpListenerOwner(ushort port)
        {
            try
            {
                // v4 listeners first, then v6 ([::1]-bound relays): either
                // family may own the port.
                var owner = FindTcpListenerOwner(port, AF_INET);
                if (owner != null) return owner;
                return FindTcpListenerOwner(port, AF_INET6);
            }
            catch { return null; }
        }

        static int? FindTcpListenerOwner(ushort port, int family)
        {
            try
            {
                uint Query(IntPtr p, ref int l) =>
