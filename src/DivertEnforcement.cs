...
namespace PTor
{
    ...
    public class DivertEnforcement : IDisposable
    {
        ...
        // 20ms (was 50ms): a rebinding app could otherwise ride the stale
        // entry long enough to push a SYN + first data out direct before
        // the revalidate drops the rest — the classic "works out of
        // nowhere after some time" shape on long-lived runs with churn.
        // Cost is one extra table walk per sustained tor flow every 20ms
        // (still far below the 100ms periodic cadence per port).
        const double AllowRevalidateMs = 20;
        ...
        static IntPtr OpenBounded(string filter, string dirForMessage)
        {
            ...
            if (!finished)
            {
                // The opener may still complete late: close its handle on
                // arrival so it can't linger with an undrained queue (which
                // would drop ITS copies and, on some driver builds, stall
                // the fresh handle opened by the retry).
                try
                {
                    _ = task.ContinueWith(t =>
                    {
                        try
                        {
                            ...
                            {
                                var late = t.Result.handle;
                                if (ValidHandle(late)) { try { DivertNative.Close(late); } catch { } }
                            }
                        }
                        catch { }
