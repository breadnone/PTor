...
namespace PTor
{
    ...
    static class MitmPipeline
    {
        ...
        {
            // Session idle bound: up to 100 keep-alive transactions share one
            // gate slot, and the header-read parks below used to wait forever
            // (same zombie class as the blind pumps). Per-transaction Relay()
            // calls + body-chunk touches below keep live sessions alive; a
            // quiet session is reaped so the slot returns to the gate.
            ...
            for (var i = 0; i < MaxTransactions; i++)
            {
                ...
                if (!await ForwardMessageBodyAsync(clientTls, upstreamTls, reqLines, method, -1, ct, opt.Touch)) return;
                ...
                opt.Relay("MITM");
                var body = await ForwardMessageBodyAsync(upstreamTls, clientTls, respLines, method, status, ct, opt.Touch);
        ...
        static async Task<bool> ForwardMessageBodyAsync(Stream from, Stream to,
            string[] headers, string method, int statusCode, CancellationToken ct, Action? touch = null)
        {
            try
            {
                if (statusCode >= 0)
                ...
                if (te.IndexOf("chunked", StringComparison.OrdinalIgnoreCase) >= 0)
                    return await ForwardChunkedBodyAsync(from, to, ct, touch);
                ...
                if (cl != null && long.TryParse(cl.Trim(), out var len))
                {
                    ...
                    return await ForwardFixedBodyAsync(from, to, len, ct, touch);
                }
                if (statusCode >= 0)
                {
        ...
        static async Task<bool> ForwardFixedBodyAsync(Stream from, Stream to, long len, CancellationToken ct, Action? touch = null)
        {
        ...
        static async Task<bool> ForwardChunkedBodyAsync(Stream from, Stream to, CancellationToken ct, Action? touch = null)
        {
            while (true)
            {
                ...
                if (!await ForwardFixedBodyAsync(from, to, size, ct, touch)) return false;
                var crlf = new byte[2];
        ...
        // Same idle-reaper discipline as StreamRelay.PumpBoth (see
        // SocksRelay.cs): a close-delimited body that stalls with no FIN
        // must not pin the caller's gate slot forever.
        static async Task PumpOneWayAsync(Stream from, Stream to, CancellationToken ct)
        {
            var buf = ArrayPool<byte>.Shared.Rent(81920);
            using var idleCts = new CancellationTokenSource(StreamRelay.IdleTimeout);
            using var linked = CancellationTokenSource.CreateLinkedTokenSource(ct, idleCts.Token);
            var pumpCt = linked.Token;
            void Touch() { try { idleCts.CancelAfter(StreamRelay.IdleTimeout); } catch { } }
            try
            {
                while (true)
                {
                    int n;
                    try { n = await from.ReadAsync(buf.AsMemory(0, 81920), pumpCt); }
                    catch { break; }
                    if (n <= 0) break;
        ...
        // including its idle reaper (an idle upgraded stream must not pin
        // the caller's gate slot forever).
