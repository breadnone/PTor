...
namespace PTor
{
    ...
    static class Obfs4Preflight
    {
        ...
        // Documentation/test ranges (RFC 5737) must never count: Snowflake
        // placeholder lines (192.0.2.x) live here. Null preflight for them
        // (fail open — the transport path decides). Never throws.
        internal static bool IsDocumentationIp(string host)
        {
            try
            {
                return host.StartsWith("192.0.2.", StringComparison.Ordinal)
