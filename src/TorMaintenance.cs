...
namespace PTor
{
    ...
    public class TorMaintenance : IDisposable
    {
        ...
        internal static TimeSpan NextRotationDelay(int intervalSec, Random? rng = null)
        {
            ...
            {
                rng ??= Random.Shared;
                // Clamp the input: absurd values (e.g. hand-edited settings)
                // must not overflow the ms math or TimeSpan below.
                var clampedSec = Math.Max(0, Math.Min(intervalSec, 86400 * 7));
                var baseMs = Math.Max(60_000L, (long)clampedSec * 1000);
                ...
                var ms = Math.Max(60_000.0, baseMs * factor);
                if (!double.IsFinite(ms)) ms = 60_000.0;
                ms = Math.Min(ms, (double)TimeSpan.MaxValue.TotalMilliseconds - 1);
                return TimeSpan.FromMilliseconds(ms);
            }
            catch { return TimeSpan.FromMinutes(1); }
        }
