...
namespace PTor
{
    ...
    public static class PinAutoRecovery
    {
        ...
        // use the entry pin is dropped from torrc (tor refuses
        // UseBridges+EntryNodes), so an entry-only pin can never be the
        // cause of a stall — rescuing (and wiping the user's entry pick)
        // would be pure harm. Returns false when there is nothing that
        // could plausibly block this run.
