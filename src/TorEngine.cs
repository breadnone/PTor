    ...
    public class TorEngine : IAsyncDisposable
    {
        ...
        async Task StartWithBridgeFallbackAsync(ResolvedBridges resolved)
        {
            ...
            foreach (var line in ordered)
            {
                ...
                // TCP preflight for IP-pinned transports (obfs4/vanilla/
                // webtunnel-style): a dead or blocked bridge otherwise burns
                // a full tor launch + bootstrap timeout (minutes) only to die
                // with tor's "Proxy Client ... handshaking (proxy) ...
                // general SOCKS server failure". A direct dial answers the
                // reachable question in seconds. Reachable is necessary, not
                // sufficient (wrong cert/DPI can still fail the handshake),
                // so reachable lines still bootstrap normally. Skipped under
                // active enforcement (probes run in the PTor process, which
                // the filter drops — every bridge would read dead).
                if (!IsSnowflakeLine(line) && !EnforcementActive)
                {
                    ThrowIfStopping();
                    var ep = Obfs4Preflight.TryParseEndpoint(line);
                    if (ep != null && !Obfs4Preflight.IsDocumentationIp(ep.Value.host))
                    {
                        var key = Obfs4Preflight.CacheKey(line);
            ...
            if (preflightSkipped > 0 && preflightSkipped >= ordered.Count)
                throw new InvalidOperationException(
                    $"None of the {ordered.Count} bridge candidate(s) are even TCP-reachable from here " +
                    "(every dial failed before tor was launched — censor block, firewall, or stale bundled bridges). " +
                    "Allow tor.exe + lyrebird.exe outbound if a firewall prompts; otherwise paste fresh private lines " +
                    "(bridges.torproject.org / @GetBridgesBot) as Custom bridges, or try Automatic/Snowflake.", last);
            throw new InvalidOperationException(
