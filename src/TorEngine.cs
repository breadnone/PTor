...
namespace PTor
{
    ...
    public class TorEngine : IAsyncDisposable
    {
        ...
        // with StrictNodes 1 + huge MaxCircuitDirtiness +
        // EnforceDistinctSubnets 0, and ALL competing
        ...
        // no UI round-trip window) and ticks auto re-enable the filter.
        // Proxy/env re-assert runs in EVERY mode (see
        // ReassertRoutingDetached): only the filter repair stays strict.
        // Volatile: set under gate by starts, read lock-free by ticks.
        ...
        public async Task<string> BuildDebugReportAsync()
        {
            ...
            try
            {
                ...
                var activeLines = ActiveBridgeLines;
                var dynamicPeers = _lastResolved?.UseBridges == true && UsesDynamicPeers(activeLines);
                sb.AppendLine(dynamicPeers
                    ? "--- tor sockets (dynamic transport; listed-IP check does not apply) ---"
                    : "--- tor sockets (non-loopback must be listed bridges) ---");
                var expected = ExpectedBridgeIps(activeLines);
                ...
                else
                {
                    ...
                    foreach (var (pid, name) in pids)
                    {
                        ...
                        foreach (var r in live)
                        {
                            ...
                            if (!ok) allOk = false;
                            sb.AppendLine($"{name}[{pid}]: {tag}" + (dynamicPeers || expected.Count == 0 ? "" : (ok ? "  [listed bridge]" : "  [NOT A LISTED BRIDGE]")));
                        }
                    }
                    if (dynamicPeers)
                        sb.AppendLine("verdict: n/a — this transport uses URL-based or dynamic endpoints; listed-bridge subset check does not apply.");
                    else if (!checkedAny)
        ...
        static HashSet<string> ExpectedBridgeIps(List<string>? lines)
        {
            ...
            try
            {
                ...
                foreach (var line in lines)
                {
                    try
                    {
                        if (!IsIpPinnedLine(line)) continue;
                        var ep = IpPinnedEndpoint(line);
                        if (ep == null || Obfs4Preflight.IsDocumentationIp(ep.Value.host)) continue;
                        set.Add(ep.Value.host);
                    }
                    catch { }
                }
            }
        ...
        static bool UsesDynamicPeers(List<string>? lines)
        {
            try
            {
                ...
                foreach (var line in lines)
                {
                    if (!IsIpPinnedLine(line)) return true;
                }
        ...
        // Effective transport of the RUNNING path ("direct" when no
        // bridges). Written at every resolve point — direct tier, winning
        // bridge candidate, reconnect — so the window title always names
        // what Tor is ACTUALLY using, including Auto-tier and rescue
        // fallbacks. Null before the first resolve of the process.
        // Volatile: written under the lifecycle gate, read lock-free by UI.
        volatile string? _activeBridgeTransport;

        void TrackActiveTransport(ResolvedBridges? resolved)
        {
            try
            {
                if (resolved?.UseBridges == true && resolved.BridgeLines != null && resolved.BridgeLines.Count > 0)
                    _activeBridgeTransport = BridgeConfigEngine.TransportOfLine(resolved.BridgeLines[0]);
                else
                    _activeBridgeTransport = "direct";
            }
            catch { }
        }

        // Mirrors the UI's selected bridge mode (+ saved lines, for Custom
        // tag accuracy) OUTSIDE starts, so the stopped-state title tag names
        // the SELECTED mode. Every StartAsync overwrites _bridges anyway.
        // Skipped while routing: the live run owns _bridges until its next
        // start (flipping it mid-run would corrupt the pin rescue gating,
        // which compares torrc reality against this field). Never throws.
        public void SyncConfiguredBridges(BridgeConfig? bridges)
        {
            try
            {
        ...
        // Maps a transport token to the title-tag body: Direct / OBFS4 /
        // SNOWFLK / WEBTUN (+ other transports verbatim, capped). Pure,
        // never throws (unit-testable).
        internal static string BridgeTagForTransport(string? transport)
        {
            try
            {
                var t = (transport ?? "").Trim();
                if (t.Length == 0 || string.Equals(t, "direct", StringComparison.OrdinalIgnoreCase))
                    return "Direct";
                if (string.Equals(t, "obfs4", StringComparison.OrdinalIgnoreCase)) return "OBFS4";
                if (string.Equals(t, "snowflake", StringComparison.OrdinalIgnoreCase)) return "SNOWFLK";
                if (string.Equals(t, "webtunnel", StringComparison.OrdinalIgnoreCase)) return "WEBTUN";
                var up = t.ToUpperInvariant();
                return up.Length <= 8 ? up : up.Substring(0, 8);
            }
            catch { return "Direct"; }
        }

        string ConfiguredBridgeTagName()
        {
            try
            {
                switch (_bridges.Mode)
                {
                    case BridgeMode.Direct: return "Direct";
                    case BridgeMode.Obfs4Default: return "OBFS4";
                    case BridgeMode.SnowflakeDefault: return "SNOWFLK";
                    case BridgeMode.WebTunnel: return "WEBTUN";
                    case BridgeMode.Auto: return "AUTO";
                    case BridgeMode.Custom:
                        try
                        {
                            var lines = _bridges.CustomLines;
                            if (lines != null && lines.Count > 0)
                                return BridgeTagForTransport(BridgeConfigEngine.TransportOfLine(lines[0]));
                        }
                        catch { }
                        return "CUSTOM";
        ...
        string EffectiveBridgeTagName()
        {
            try
            {
                // Plain read: the field is volatile, so this is already a
                // volatile read (Volatile.Read on a volatile field only adds
                // a CS0420 warning, not semantics).
                var t = _activeBridgeTransport;
                if (string.IsNullOrWhiteSpace(t)) return ConfiguredBridgeTagName();
                return BridgeTagForTransport(t);
            }
            catch { return "Direct"; }
        }

        // Window-title tag for the bridge path, e.g. "[SNOWFLK]".
        // CRITICAL fallback rule: when the live path differs from the
        // configured mode (an Auto tier winning something else, a rescue
        // landing elsewhere), the tag names the EFFECTIVE path prefixed
        // with FB — "[FB:SNOWFLK]". Same transport from another source
        // (fresh fetch vs cache vs bundled) is NOT a fallback: the mode
        // didn't change. Routing off shows the configured mode (nothing is
        // active). Never throws.
        public string ActiveBridgeTag
        {
            get
            {
                try
                {
                    var configured = ConfiguredBridgeTagName();
                    if (!RoutingActive) return "[" + configured + "]";
                    var effective = EffectiveBridgeTagName();
                    if (!string.Equals(effective, configured, StringComparison.Ordinal))
                        return "[FB:" + effective + "]";
                    return "[" + effective + "]";
                }
                catch { return ""; }
            ...
            get
            {
                try
                {
                    ...
                    return $"Bridges: {active} · transport processes: {ptCount} · bundled defaults: " +
                        (has ? $"obfs4×{o}, snowflake×{s}" : "missing (automatic fetch still tried at start)") +
        ...
        // Fast routing-intact watcher: CheckRoutingIntact is cheap
        // (registry reads + a divert state peek), while the link tick above
        // is a ~20s network fetch every ~30s. A GPO refresh / VPN connect /
        // new NIC that stomps the proxy used to leak for up to 30s before
        // even warning (and forever in proxy-only, which never repaired).
        // This 5s cadence bounds the divergence window to seconds in every
        // mode. Started/stopped with the link checks; warnings inside stay
        // throttled so the cadence is silent when healthy.
        Timer? _intactTimer;
        int _intactBusy;
            ...
            bool requireLockdown)
        {
            ...
            // Same guess as the start log below: an entry-only pin under
            // bridges constrains nothing, so the geo/dirtiness fields must
            // survive for the torrc to use. torrc re-decides with the ACTUAL
            // resolve (exact, not a guess) and a guess-effective pin always
            // constrains for real — so keeping fields here can only help,
            // never leak an override past an effective pin.
            var pinConstrainsGuess = pinActive && ((_bridges.Mode == BridgeMode.Direct)
                || (pinMiddle.Count + pinExit.Count) > 0);
            ...
            try
            {
                if (pinActive)
                {
                    // torrc truth, pre-resolve guess: an entry-only pin under
                    // bridges constrains nothing (entry line dropped, no
                    // StrictNodes, normal lifetime — see BuildTorrcContent).
                    // Auto counts as bridged here; if it wins the direct tier
                    // the torrc still applies the entry pin — the guess can
                    // only under-claim in the log, never over-claim.
                    bool constrainsGuess = (_bridges.Mode == BridgeMode.Direct)
                        || (pinMiddle.Count + pinExit.Count) > 0;
                    LogMessage?.Invoke(this,
                        ...
                        (constrainsGuess
                            ...
                            : "(entry-only pin under bridges constrains NOTHING this run — normal lifetime kept). ") +
                        "Overrides: stable mode, circuit lifetime, exit geography, scheduled + manual rotation.");
                    // The pin constrains WHICH relays Tor may use — it does
                    // not freeze Tor's circuit lifecycle (circuits still die
                    // and rebuild). Name the configurations that still hop so
                    // a later IP change is never a mystery.
                    try
                    {
                        foreach (var w in TorProcessManager.PinStabilityWarnings(pinEntry, pinMiddle, pinExit))
                            LogMessage?.Invoke(this, "Circuit pin stability: " + w);
                    }
                    catch { }
                    if ((_bridges.Mode != BridgeMode.Direct) && pinEntry.Count > 0)
                        LogMessage?.Invoke(this,
                            "Note: bridges are in use — the bridge IS the entry, so the entry pin is dropped from torrc for this run " +
                            "(tor refuses to start with both UseBridges and EntryNodes). It stays saved and applies again on direct runs; " +
                            "middle/exit pins still apply now.");
                    if (rotateEverySec > 0)
                        LogMessage?.Invoke(this,
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (_bridges.Mode == BridgeMode.Direct)
                    {
                        ...
                        await StartCandidateAsync(null);
                    ...
                    else
                    {
                        ...
                        try { resolved = await ResolveBridgesLiveFirstAsync(_bridges, bridgeDefaults); }
        ...
        // in use the entry pin is dropped from torrc (tor refuses
        // UseBridges+EntryNodes), so an entry-only pin can never stall the
        // bootstrap and must never trigger a pin rescue (which would wipe a
        // pick that was never the problem). Direct runs count every
        // position. Never throws.
        //
        // Bridge-awareness follows the ACTUAL run, not the configured mode:
        // under Automatic the winning tier may be direct (entry pin applies)
        // or bridged (entry dropped). _lastResolved names the winner from
        // the first launch of the run; before that, the configured mode is
        // the only hint (Auto counts as bridged — the guess that can only
        // defer a rescue, never wipe a healthy pin early).
        bool RunUsesBridges()
        {
            try
            {
                var r = _lastResolved;
                if (r != null) return r.UseBridges;
                return _bridges.Mode != BridgeMode.Direct;
            }
            catch { return _bridges.Mode != BridgeMode.Direct; }
        }
        ...
        bool BootstrapPinActive()
        {
            try
            {
                ...
                if (RunUsesBridges()) return (middle + exit) > 0;
                return true;
            }
            catch { return false; }
        }
        ...
        async Task StartCoreInnerAsync(int rotateEverySec)
        {
            ...
            else
            {
                ...
                try
                {
                    resolved = await ResolveBridgesLiveFirstAsync(_bridges, bridgeDefaults);
                }
                catch (Exception ex)
                {
        ...
        // Cold datadir (first boot): tor has no cached consensus/microdescs
        // yet, so the first working bridge must download megabytes through
        // obfs4 before bootstrap can proceed — legitimately slower than
        // steady state, where the same bridge would connect inside the base
        // budget. Detected by the absence of tor's cache files; the caller
        // adds ColdBootstrapBonusSec to that run's per-candidate budget only
        // (steady state keeps the tight timeout so dead bridges still fail
        // fast). Pure + never throws.
        internal const int ColdBootstrapBonusSec = 120;

        internal static bool IsColdConsensusCache(string? dataDir)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(dataDir)) return false;
                foreach (var name in new[] { "cached-consensus", "cached-microdesc-consensus", "cached-microdescs" })
                {
                    try { if (File.Exists(Path.Combine(dataDir, name))) return false; }
                    catch { }
                }
                return true;
            }
        ...
        async Task WaitForBootstrapCompleteAsync(int timeoutSec, string what, int stallFreezeSec = 60, bool pinActive = false, bool allowEarlyStallAbort = true)
        {
            ...
            while (sw.Elapsed.TotalSeconds < timeoutSec)
            {
                ...
                if (pinActive && pinHits >= 2 &&
                    ...
                    throw new PinSuspectBootstrapException(
                        ...
                        pinEvidence, true);
                if (allowEarlyStallAbort && sw.Elapsed.TotalSeconds >= 60 && frozenSec >= stallFreezeSec && handshakeDeaths >= 3)
                {
                    throw new TimeoutException(StallAbortMessage(what, lastPct, handshakeDeaths, clockHints, skew));
                }

                if (allowEarlyStallAbort && sw.Elapsed.TotalSeconds >= 90 && frozenSec >= 90 && handshakeDeaths == 0)
                {
        ...
        // Live-first resolve for the automatic modes: fresh directory fetch
        // → last-good auto cache (recent only) → bundled pt_config.json.
        // The bundled file is the LAST resort, never the first choice, so
        // its aging lines stop mattering the moment anything better exists.
        // Applies to the obfs4 / Snowflake defaults and to WebTunnel runs
        // with no saved lines; WebTunnel runs WITH saved lines, Custom and
        // Direct never fetch — saved lines are authoritative there. Honors
        // stop (a stop racing the fetch aborts instead of falling back).
        // Never throws except on stop: the worst case is the same bundled
        // resolve as before.
        async Task<ResolvedBridges> ResolveBridgesLiveFirstAsync(BridgeConfig cfg, PtDefaults? bridgeDefaults)
        {
            try
            {
                cfg ??= BridgeConfig.DirectOnly();
                string? transport = null;
                if (cfg.Mode == BridgeMode.Obfs4Default) transport = "obfs4";
                else if (cfg.Mode == BridgeMode.SnowflakeDefault) transport = "snowflake";
                else if (cfg.Mode == BridgeMode.WebTunnel && (cfg.CustomLines == null || cfg.CustomLines.Count == 0))
                    transport = "webtunnel";
                if (transport == null)
                    return BridgeConfigEngine.Resolve(cfg, PtToolsDir, bridgeDefaults);
                var ct = _lifetimeCts?.Token ?? CancellationToken.None;

                // 1) Fresh directory fetch (one quick try, short budget).
                BridgeFetchResult? live = null;
                try
                {
                    using var client = BridgeFetcher.CreateClient(BridgeFetcher.StartRefreshTimeout);
                    live = transport == "snowflake"
                        ? await BridgeFetcher.FetchBuiltinSnowflakeAsync(client, ct)
                        : await BridgeFetcher.FetchTransportAsync(client, transport, ct);
                }
                catch (OperationCanceledException) when (ct.IsCancellationRequested) { throw; }
                catch (OperationCanceledException) { live = null; }
                catch { live = null; }
                if (live != null && live.Lines.Count > 0)
                {
                    try { AutoBridgeCache.Save(transport, live.Lines); } catch { }
                    var via = ResolveAutoLines(live.Lines, bridgeDefaults);
                    if (via.UseBridges)
                    {
                        var label = "live " + transport + " (fetched)";
                        try { LogMessage?.Invoke(this, $"Bridges: using {via.BridgeLines.Count} fresh {label} from Tor's directory — cache updated, bundled file skipped."); } catch { }
                        return via with { Label = label };
                    }
                    try { LogMessage?.Invoke(this, "Bridges: fetched lines unusable (" + string.Join(" ", via.Errors) + ") — trying cache, then bundled."); } catch { }
                }
                else
                {
                    try { LogMessage?.Invoke(this, "Bridges: live refresh unavailable (" + (live?.Error ?? "no result") + ") — trying cache, then bundled."); } catch { }
                }

                // 2) Last-good auto cache (recent only, matching transport).
                AutoBridgeCache.CachedBridges? cached = null;
                try { cached = AutoBridgeCache.Load(); } catch { }
                if (cached != null && string.Equals(cached.Transport, transport, StringComparison.OrdinalIgnoreCase)
                    && cached.Lines.Count > 0 && AutoBridgeCache.IsFresh(cached, DateTime.UtcNow))
                {
                    var viaCache = ResolveAutoLines(cached.Lines, bridgeDefaults);
                    if (viaCache.UseBridges)
                    {
                        var age = AutoBridgeCache.DescribeAge(cached, DateTime.UtcNow);
                        try { LogMessage?.Invoke(this, $"Bridges: directory unreachable — using {viaCache.BridgeLines.Count} cached {transport} lines ({age}), bundled file skipped."); } catch { }
                        return viaCache with { Label = $"cached {transport} (auto, {age})" };
                    }
                    try { LogMessage?.Invoke(this, "Bridges: cached lines unusable — using bundled defaults."); } catch { }
                }

                // 3) Bundled file.
                try { LogMessage?.Invoke(this, "Bridges: using bundled defaults."); } catch { }
                return BridgeConfigEngine.Resolve(cfg, PtToolsDir, bridgeDefaults);
            }
            catch (OperationCanceledException) { throw; }
            catch { return BridgeConfigEngine.Resolve(cfg, PtToolsDir, bridgeDefaults); }
        }

        // Resolves fetched/cached lines through the Custom path (plugin
        // lines included). Never throws: failures read as UseBridges=false.
        ResolvedBridges ResolveAutoLines(List<string> lines, PtDefaults? bridgeDefaults)
        {
            try
            {
                return BridgeConfigEngine.Resolve(
                    new BridgeConfig(BridgeMode.Custom, lines ?? new List<string>()),
                    PtToolsDir, bridgeDefaults);
            }
            catch
            {
                return new ResolvedBridges(false, new List<string>(), new List<string>(),
                    new List<string> { "resolve failed" }, "custom");
            }
        }
        ...
        async Task StartWithBridgeFallbackAsync(ResolvedBridges resolved)
        {
            ...
            // Say the retry state out loud: every failed run benches what
            // just died, so successive retries walk down the list — a retry
            // that "suddenly works" usually just reached a live bridge, not
            // whatever setting was toggled in between.
            try
            {
                var snap = _bridgeHealth.Snapshot();
                var benched = ordered.Count(l =>
                {
                    try
                    {
                        return snap.TryGetValue(BridgeHealthStore.KeyOf(l), out var e) && e?.DeadSinceUtc != null;
                    }
                    catch { return false; }
                });
                if (benched > 0)
                    LogMessage?.Invoke(this,
                        $"{benched}/{ordered.Count} bridge candidate(s) benched from prior failures — trying the rest first.");
            }
            catch { }
            // First-boot budget: with a cold Tor cache the first working
            // bridge must fetch the whole consensus through obfs4. Slow but
            // healthy bridges would otherwise burn the base timeout and get
            // benched for being slow on the one run that is slow for everyone.
            int coldBonus = 0;
            try
            {
                if (IsColdConsensusCache(Path.Combine(_appDir, "tordata")))
                {
                    coldBonus = ColdBootstrapBonusSec;
                    LogMessage?.Invoke(this,
                        "Cold Tor cache (first boot): bridge candidates get extra time for the initial consensus download.");
                }
            }
            catch { coldBonus = 0; }
            StartRelays();
            ...
            var obfs4PreflightCache = new Dictionary<string, (bool ok, string reason)>(StringComparer.OrdinalIgnoreCase);
            ...
            foreach (var line in ordered)
            {
                ...
                if (IsSnowflakeLine(line) && !EnforcementActive)
                {
                    ...
                    if (!preflightCache.TryGetValue(key, out var verdict))
                    {
                        ...
                        {
                            verdict = (false, "diagnostic unavailable or timed out");
                        }
                        ct.ThrowIfCancellationRequested();
                    ...
                    if (!verdict.ok)
                    {
                        // Deterministic death (tor would print the same
                        // verdict after minutes: "broker failure ... lookup"
                        // then "stuck at 10%"): skip WITHOUT a tor launch
                        // instead of burning the full ~7-minute rendezvous
                        // budget per candidate. Anything less certain keeps
                        // the full attempt below.
                        bool hard = false;
                        try { hard = SnowflakePreflight.IsHardFailure(verdict.reason); } catch { }
                        if (hard)
                        {
                            _bridgeHealth.RecordResult(line, false);
                            preflightSkipped++;
                            var leftHard = ordered.Count - tried;
                            try
                            {
                                LogMessage?.Invoke(this,
                                    $"Snowflake {BridgeHealthStore.ShortName(line)} unusable ({verdict.reason}) — " +
                                    "skipping without a tor launch (no multi-minute stall" +
                                    (leftHard > 0 ? $", {leftHard} left)." : ", none left)."));
                            }
                            catch { }
                            ThrowIfStopping();
                            continue;
                        ...
                        {
                            LogMessage?.Invoke(this,
                                "Snowflake preflight inconclusive: DNS/STUN probes did not confirm connectivity. " +
                                "This does not establish whether the helper can connect; trying Snowflake with its full rendezvous budget.");
                        }
                        catch { }
                    }
                }
                if (IsIpPinnedLine(line) && !EnforcementActive)
                {
                    ThrowIfStopping();
                    var ep = IpPinnedEndpoint(line);
                    if (ep != null && !Obfs4Preflight.IsDocumentationIp(ep.Value.host))
                    {
                        var key = $"{ep.Value.host}:{ep.Value.port}";
                        if (!obfs4PreflightCache.TryGetValue(key, out var tcpVerdict))
                        {
                            tcpVerdict = await Obfs4Preflight.CheckAsync(
                                ep.Value.host, ep.Value.port,
                                Obfs4Preflight.TryTcpConnectAsync,
                                TimeSpan.FromSeconds(8),
                                _lifetimeCts?.Token ?? CancellationToken.None);
                            obfs4PreflightCache[key] = tcpVerdict;
                        }
                        if (!tcpVerdict.ok)
                        {
                            preflightSkipped++;
                            ...
                            LogMessage?.Invoke(this,
                                $"Bridge candidate {BridgeHealthStore.ShortName(line)} unreachable ({tcpVerdict.reason}) — " +
                                (left > 0 ? $"skipping without a tor launch ({left} left)." : "skipping without a tor launch (none left)."));
                            continue;
                        }
                    }
                }
                ...
                try
                {
                    await StartCandidateAsync(single);
                }
                catch (OperationCanceledException) { throw; }
                catch (Exception ex)
                {
                ...
                try
                {
                    ...
                    await WaitForBootstrapCompleteAsync(BootstrapTimeoutForLine(line) + coldBonus,
                        BridgeHealthStore.ShortName(line), stallFreezeSec: 120, pinActive: BootstrapPinActive(),
                        allowEarlyStallAbort: !IsSnowflakeLine(line));
            ...
            if (preflightSkipped > 0 && preflightSkipped >= ordered.Count)
                throw new InvalidOperationException(
                    $"None of the {ordered.Count} bridge candidate(s) are reachable from here " +
                    "(every check failed before tor was launched — censor block, firewall, broken system DNS, or blocked UDP for Snowflake). " +
                    "Allow tor.exe + lyrebird.exe outbound if a firewall prompts; Snowflake needs working system DNS and direct UDP. " +
                    "Otherwise try Automatic or a different bridge mode.", last);
            throw new InvalidOperationException(
                "All bridge candidates failed" +
                (ordered.Count > 1 ? $" ({ordered.Count} tried" +
                    (preflightSkipped > 0 ? $", {preflightSkipped} unreachable-skipped" : "") +
                    ", dead ones are benched for next time)" : "") +
                ". " + ShortError(last) +
                " If the bridges are stale, fetch fresh ones (Config → bridges → Fetch now) or try Automatic.", last);
        }
        ...
        internal static bool IsIpPinnedLine(string? line)
        {
            var t = BridgeConfigEngine.TransportOfLine((line ?? "").Trim());
            return string.Equals(t, "obfs4", StringComparison.OrdinalIgnoreCase)
                || string.Equals(t, "obfs3", StringComparison.OrdinalIgnoreCase)
                || string.Equals(t, "scramblesuit", StringComparison.OrdinalIgnoreCase)
                // WebTunnel endpoints are literal IPs too (validation
                // requires IP:port + fingerprint), so dead bridges fail fast
                // on a TCP dial instead of burning a full bootstrap per
                // candidate. Snowflake is deliberately EXCLUDED: its
                // 192.0.2.x endpoint is a dummy (rendezvous is via broker
                // fronts + UDP) — dialing it would skip every working line.
                || string.Equals(t, "webtunnel", StringComparison.OrdinalIgnoreCase)
                || (t.Contains(':') && IpPinnedEndpoint(line) != null);
        }

        static (string host, int port)? IpPinnedEndpoint(string? line)
        {
            var normalized = (line ?? "").Trim();
            var t = BridgeConfigEngine.TransportOfLine(normalized);
            var ep = Obfs4Preflight.TryParseEndpoint(t.Contains(':') ? "vanilla " + normalized : normalized);
            return ep != null && IPAddress.TryParse(ep.Value.host, out _) ? ep : null;
        }
        ...
        async Task StartAutoTierAsync(PtDefaults? defaults)
        {
            foreach (var (mode, label) in AutoTiers())
            {
                ...
                // Default tiers resolve live-first (quick directory try,
                // bundled fallback); custom lines are used as-is.
                var resolved = await ResolveBridgesLiveFirstAsync(cfg, defaults);
                if (!resolved.UseBridges)
                {
                    LogMessage?.Invoke(this,
        ...
        async Task StartCandidateAsync(ResolvedBridges? resolved)
        {
            ThrowIfStopping();
            try { _maintenance?.Dispose(); } catch { }
            _maintenance = null;
            ...
            // Title source of truth: every tor launch (direct tier, bridge
            // candidate, reconnect) passes here with the exact resolve tor
            // is about to run — null means direct guards.
            try { TrackActiveTransport(resolved); } catch { }
            _proc.Start(resolved);
        }
        ...
        async Task HandleUnexpectedExit()
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    await StartCandidateAsync(last?.UseBridges == true ? last : null);
                }
                finally
                {
                    _lifecycleGate.Release();
                ...
                try
                {
                    var snowflake = rl?.UseBridges == true && rl.BridgeLines.Any(IsSnowflakeLine);
                    if (snowflake) bsTimeout = Math.Max(bsTimeout, 420);
                    await WaitForBootstrapCompleteAsync(bsTimeout, bsWhat, stallFreezeSec: bsFreeze, pinActive: pinActive,
                        allowEarlyStallAbort: !snowflake);
                }
                catch (OperationCanceledException)
                {
                    try { await _proc.StopAsync(); } catch { }
        ...
        public async Task<(bool ok, string message)> RotateNowAsync()
        {
            ...
            if (_pinnedCircuitActive)
                return (false, "New identity is disabled while circuit pin is on — turn the pin off in Config and restart Tor (toggle off/on) to rotate (pin keeps one path to stop IP hops).");
            if (_maintenance == null)
        ...
        void StartLinkChecks()
        {
            ...
            // Fast intact watcher (see field comment): 5s fixed cadence,
            // first run immediate so a start-time divergence is caught now,
            // not on the next ~30s link tick.
            try
            {
                try { _intactTimer?.Dispose(); } catch { }
                _intactTimer = new Timer(_ =>
                {
                    if (System.Threading.Interlocked.CompareExchange(ref _intactBusy, 1, 0) != 0) return;
                    try { CheckRoutingIntact(); }
                    catch { }
                    finally { System.Threading.Interlocked.Exchange(ref _intactBusy, 0); }
        ...
        async Task LinkTickCore()
        {
            ...
            catch (Exception ex)
            {
                ...
                if (_linkFails >= LinkFailThreshold)
                {
                    ...
                    // excluded (the entry line is dropped from torrc there —
                    // the entry pick can never be the cause).
                    if (_pinnedCircuitActive)
                    {
                        if (!RuntimePinEffective())
                        {
                            LogMessage?.Invoke(this, "Tor data path degraded — rotation skipped (circuit-pin mode keeps one path). Entry pin is dropped while bridges are in use, so no pin recovery was triggered.");
                        }
                        else
                        {
                            _pinDegradedLadders++;
        ...
        public Task ResumeRecoveryAsync()
        {
            return Task.Run(async () =>
            {
                try
                {
                    ...
                    // path with ISP resolvers AND a stomped proxy: re-assert
                    // NOW instead of waiting for the next tick. Proxy/env
                    // put-back runs in every mode (a diverged proxy leaks
                    // RIGHT NOW while the link probe below takes 12s);
                    // DNS newcomer fix still needs the forwarder.
                    try
                    {
        ...
        void CheckRoutingIntact()
        {
            try
            {
                ...
                // Self-heal in EVERY mode (not just strict): put back what
                // was overwritten externally (GPO/VPN/another tool) instead
                // of warn-only. Proxy-only runs used to warn forever while
                // the leak stayed open for hours — the "after some time"
                // report. Runs on the tick thread (registry writes only,
                // no gate); DNS put-back still needs the forwarder (gated
                // inside ReassertRoutingDetached).
                if (!intact)
                {
                    try { ReassertRoutingDetached(); } catch { }
                    try
                ...
                if (!intact && !_routingDiverged)
                {
                    ...
                    LogMessage?.Invoke(this,
                        ...
                        "re-asserted PTor's settings automatically; if you changed them on purpose, toggle routing off/on. " +
                        "Tor-only lockdown additionally blocks non-proxy/UDP bypasses that re-asserting can't reach.");
                }
                else if (!intact && _routingDiverged)
                {
                    // Still diverged (re-assert keeps failing — e.g. GPO
                    // fighting back every refresh): re-warn every ~5 min
                    // (intact watcher runs every 5s) instead of staying
                    // silent for hours.
                    try
                    {
                        if ((DateTime.UtcNow - _lastDivergeWarnUtc).TotalMinutes >= 5)
                        {
                            _lastDivergeWarnUtc = DateTime.UtcNow;
                            LogMessage?.Invoke(this,
                                "WARNING (still diverged): system proxy/env/DNS still differ from PTor's settings despite auto-repair — " +
        ...
        string EnableEnforcementCore(bool allowPreRouting = false)
        {
            // Caller holds gate + enforce lock; guard below is backstop.
            {
                ...
                {
                    // dns may have failed after construction but before being
                    // published to _dnsFwd (e.g. :53 bind race): dispose the
                    // local too, or it keeps the port while we report OFF.
                    try { dns?.Dispose(); } catch { }
                    try { _dnsFwd?.Dispose(); } catch { }
                    _dnsFwd = null;
            ...
            try
            {
                ...
                _ = Task.Run(() =>
                {
                    ...
                    try
                    {
                        ...
                        if (!taken)
                        {
                            // Gate busy (start/stop in flight): don't consume
                            // the 60s throttle — roll back so the next tick
                            // retries immediately after the gate frees
                            // instead of leaking for a full minute.
                            try { _lastEnforceRepairUtc = DateTime.MinValue; } catch { }
                            return;
                        }
                        try
                        {
        ...
        // Routing re-assert (pool thread, registry writes only, no gate):
        // rewrites APPLIED proxy/env/DNS without touching backups, plus
        // newcomer NICs. Proxy/env re-assert runs in EVERY mode (not just
        // strict): a diverged proxy with routing ON sends proxy-aware apps
        // direct RIGHT NOW — the classic "worked, then leaked out of
        // nowhere after some time" shape (GPO refresh ~90min, VPN connect,
        // MDM/security tool stomping HKCU). DNS re-assert needs the
        // filter's forwarder, so it stays gated on EnforcementActive.
        // Never throws.
