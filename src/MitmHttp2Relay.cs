...
namespace PTor
{
    ...
    public sealed class MitmSessionOptions
    {
        ...
        // Session liveness hook for the idle reaper (see SessionIdleScope):
        // set at session start, invoked on every relayed message/frame so a
        // slow-but-moving stream is never mistaken for a zombie. Null when
        // the session runs without an idle bound. Never throws by contract
        // (callers invoke defensively anyway).
        public Action? Touch;
        ...
        {
            // Telemetry doubles as the liveness signal: every relayed
            // message/frame proves the session is moving.
            try { Touch?.Invoke(); } catch { }
    ...
    public static class MitmHttp2Relay
    {
        ...
        {
            // Session idle bound (same zombie class as the blind pumps):
            // every forwarded frame reports via opt.Relay below, which
            // touches the scope; a frameless session is reaped. Existing
            // awaits propagate the abort as teardown.
            using var idle = new SessionIdleScope(ct);
            opt.Touch = idle.Touch;
            ct = idle.Token;
        ...
        public static async Task RunH2ToH1Async(Stream clientT, MitmSessionOptions opt, CancellationToken ct)
        {
            ...
            // Session idle bound (see RunAsync): client frame reads below
            // touch per frame; translator bodies touch per chunk; everything
            // else aborts as teardown when quiet.
            using var idle = new SessionIdleScope(ct);
            opt.Touch = idle.Touch;
            ct = idle.Token;
            var client = await AcceptClientAsync(clientT, ct);
            ...
            MitmSessionOptions opt, CancellationToken ct)
        {
            ...
            while (true)
            {
                ...
                // Any client frame (DATA upload, HEADERS, even PING) proves
                // the session is alive. Downloads touch via opt.Relay.
                try { opt.Touch?.Invoke(); } catch { }
                switch (f.Type)
                {
                    case Http2Const.SETTINGS:
                        if (f.StreamId != 0) throw new Http2Exception(Http2Const.PROTOCOL_ERROR, "SETTINGS on stream.");
            ...
            ConcurrentDictionary<int, StreamInbox> inboxes, MitmSessionOptions opt, CancellationToken ct)
        {
            ...
            try
            {
                ...
                using (tcp)
                {
                    ...
                    if (!noBody)
                        await ForwardH1BodyToH2Async(upTls, client, streamId, respLines, req.Method, status, respExtra, ct, opt.Touch);
        ...
        static async Task ForwardH1BodyToH2Async(Stream h1, Http2Leg client, int streamId,
            string[] respLines, string method, int status, byte[] prefix, CancellationToken ct, Action? touch = null)
        {
            ...
            if (chunked)
            {
                await ForwardH1ChunkedToH2Async(reader, client, streamId, ct, touch);
                return;
            }
            ...
            if (cl != null && long.TryParse(cl.Trim(), out var len) && len > 0)
            {
                ...
                try
                {
                    ...
                    while (left > 0)
                    {
                        ...
                        // Slow-but-moving bodies must not look idle to the reaper.
                        try { touch?.Invoke(); } catch { }
                    }
                }
        ...
        static async Task ForwardH1ChunkedToH2Async(PrefixedBodyReader reader, Http2Leg client, int streamId,
            CancellationToken ct, Action? touch = null)
        {
        ...
        {
            // Session idle bound (see RunAsync): per-message Relay() calls
            // below touch the scope; a quiet session is reaped.
            using var idle = new SessionIdleScope(ct);
            opt.Touch = idle.Touch;
            ct = idle.Token;
            if (opt.SocksDial == null || opt.AuthenticateUpstream == null)
        ...
        static async Task H1ToH2LoopAsync(Stream clientT, Http2Leg up, MitmSessionOptions opt, CancellationToken ct)
        {
            ...
            while (true)
            {
                ...
                if (hasBody || reqExtra.Length > 0)
                    await ForwardH1BodyToH2StreamAsync(clientT, up, sid, reqLines, reqExtra, ct, opt.Touch);
                ...
                while (!gotFinal)
                {
                    ...
                    if (!noBody && !end)
                        await ForwardH2StreamToH1ChunkedAsync(up, clientT, sid, method, ct, opt.Touch);
        ...
        static async Task ForwardH1BodyToH2StreamAsync(Stream h1, Http2Leg up, int sid,
            string[] reqLines, byte[] prefix, CancellationToken ct, Action? touch = null)
        {
        ...
        static async Task ForwardH2StreamToH1ChunkedAsync(Http2Leg up, Stream h1out, int sid, string method, CancellationToken ct, Action? touch = null)
        {
